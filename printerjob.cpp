	...
	if (OnPreparePrinting(&printInfo))
	{
		...
		for (printInfo.m_nCurPage = nStartPage; printInfo.m_nCurPage != nEndPage; printInfo.m_nCurPage += nStep)
		{
			...
			if (tck > 60)
			{
				...
				DWORD tckS = tck%60;
				tck /= 60;
				strTemp.Format("%d ,minuto, %d segundo", tck, tckS);
			}
			else
			{
				if (iPageTot > 1)
				{
					if (tck > 0) strTemp.Format("%d segundo", tck);
					else strTemp = "??";
				}
			}
