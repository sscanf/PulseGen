...
void COScopeCtrl::InvalidateCtrl()
{
  ...
  m_nCurrentXPosition = m_rectPlot.left + m_nPlotShiftPixels;	//starting x position for Oscilloscope mode
...
void COScopeCtrl::DrawPoint()
  {
	...
	if(m_nScopeMode == CHART_RECORDER_MODE) {
	...
	{ // m_nScopeMode = OSCILLOSCOPE_MODE
		// this part, just clears a narrow slit of m_dcPlot with the background color
		// and then draws the new point there
		currX =int((double(m_nCurrentXPosition)/float(X_POINTS_PER_PLOT))*double(m_rectPlot.right - m_rectPlot.left)) +m_rectPlot.left;
		prevX =int((double(m_nCurrentXPosition-1)/float(X_POINTS_PER_PLOT))*double(m_rectPlot.right - m_rectPlot.left)) +m_rectPlot.left;
  
		m_dcPlot.BitBlt(currX, m_rectPlot.top+1, 
						3, m_nPlotHeight, NULL, NULL, NULL, 
						WHITENESS) ;
		// do a fill here?
		rectCleanUp = m_rectPlot ;
		rectCleanUp.left  = currX;
		rectCleanUp.right = currX+3;
		rectCleanUp.bottom += 1;
  
	    // fill the cleanup area with the background
	    m_dcPlot.FillRect(rectCleanUp, &m_brushBack) ;
  
		// now plot the next line segment
	    prevY = m_rectPlot.bottom - 
		   (long)((m_dPreviousPosition - m_dLowerLimit) * m_dVerticalFactor) ;
	    currY = m_rectPlot.bottom -
				(long)((m_dCurrentPosition - m_dLowerLimit) * m_dVerticalFactor) ;
	    ...
	    m_dcPlot.LineTo (currX, currY) ;
		m_nCurrentXPosition++;
		if(m_nCurrentXPosition>X_POINTS_PER_PLOT-1)m_nCurrentXPosition = 0;
